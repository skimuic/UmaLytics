...
function run({privateBuild=false, existing, uploadFails=false, listFails=false, wrongTag=false, noTag=false}={}) {
  ...
  const context={root,path,createHash,console:{log(){}},process:{env:{GITHUB_REPOSITORY:'kjunodev/umalytics',GITHUB_SHA:sha}},
    ...
    execFileSync:(cmd,args)=>{calls.push([cmd,...args]);if(cmd==='zip'){files.set(args[1],'ZIP');return '';}
      if(args[0]==='api'){
...
test('release resumes an unpublished untagged draft after a publishing fix',()=>{const r=run({noTag:true,existing:{tag_name:'v0.4.0-open-beta.1',draft:true,target_commitish:'b'.repeat(40),assets:[]}});assert.equal(r.error,undefined);assert(r.calls.some(c=>c[2]==='edit' && c.includes('--target') && c.includes(r.sha)));assert(r.calls.some(c=>c[2]==='edit' && c.includes('--draft=false')));});
